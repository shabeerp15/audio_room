		...
		<StreamVideo client={client!}>
			<div className="home">
				...
				<div className="form">
					...
					<input placeholder="Room Name..." onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewRoom((prev) => ({ ...prev, name: e.target.value }))} />
					<input placeholder="Room Description..." onChange={(e: React.ChangeEvent<HTMLInputElement>) => setNewRoom((prev) => ({ ...prev, description: e.target.value }))} />
				...
				<div className="grid">
					{rooms.map((room) => (
						<div className="card" key={room.id} onClick={() => joinRoom(room.id)}>
							<h4>{room.title}</h4>
